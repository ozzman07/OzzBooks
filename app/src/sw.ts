...
import { deleteStaleV2Cache } from './offline/audioChunkStore'
...
// One-time cleanup of a Cache Storage bucket a previous, reverted attempt
// at this feature used — see audioChunkStore.ts's doc comment.
self.addEventListener('activate', (event) => {
  event.waitUntil(deleteStaleV2Cache())
})

// Range-request serving for downloaded audiobooks, chunked throughout (see
// offlineAudioRange.ts / audioChunkStore.ts for why: avoids handing
// <audio> one giant Blob URL or making the service worker materialize a
// whole cached file at once, either of which caused silent,
// unrecoverable stalls/crashes on memory-constrained iOS devices for
