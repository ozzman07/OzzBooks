...
import { getCachedAudioFile, migrateLegacyAudioBlobIfNeeded, offlineAudioUrl, touchLastPlayed } from '../offline/audioFileStore'
...
export function PlayerProvider({ children }: { children: ReactNode }) {
   ...
   *
   * migrateLegacyAudioBlobIfNeeded is a no-op for anything downloaded
   * after this change; for a book downloaded before it (whose blob is
   * still sitting directly in the IndexedDB row), it moves the blob into
   * Cache Storage on this first play — see audioFileStore.ts. */
  const resolveAudioSrc = useCallback(async (target: Chapter): Promise<string> => {
    ...
    await migrateLegacyAudioBlobIfNeeded(cached)
